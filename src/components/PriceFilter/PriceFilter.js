...
function PriceFilter() {
  ...
  const handleMouseMove = (e) => {
    ...
    if (isDraggingLeft) {
      ...
      leftRef.current.style.left = `${Math.max(
        0,
        Math.min(newLeft, rightRef.current.offsetLeft - 10)
      )}px`;

      let percentage = Math.floor((newLeft / wrapperWidth) * 100);

      if (
        maxPrice * (percentage / 100) >= 0 &&
        maxPrice * (percentage / 100) <= 900
      ) {
    ...
    if (isDraggingRight) {
      ...
      if (
        maxPrice * (percentage / 100) >= 100 &&
        maxPrice * (percentage / 100) <= 1000
      ) {
